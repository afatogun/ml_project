...
def main() -> int:
    parser = argparse.ArgumentParser(description="Clean likely mislabeled Miscellaneous rows")
    parser.add_argument(
        "--train_xlsx",
        default="./data/raw/planningdata_for_classification_20251219.csv",
        help="Path to training dataset (CSV or XLSX)",
    )
    parser.add_argument(
        "--cleaned_output",
        default="./data/raw/planningdata_for_classification_20251219.cleaned.csv",
        help="Path to save cleaned dataset (CSV or XLSX)",
    )
    parser.add_argument(
        "--removed_output",
        default="./data/raw/misc_removed.csv",
        help="Path to save removed rows report (CSV)",
    )
    parser.add_argument(
        "--confidence_threshold",
        type=float,
        default=0.8,
        help="Confidence threshold for flagging mislabeled Miscellaneous rows",
    )
    ...
    confident_mask_b = (split_b_results["pred_sector"].str.lower() != "miscellaneous") & (
        split_b_results["pred_sector_conf"] >= args.confidence_threshold
    )
    ...
    confident_mask_a = (split_a_results["pred_sector"].str.lower() != "miscellaneous") & (
        split_a_results["pred_sector_conf"] >= args.confidence_threshold
    )
    ...
    # Ensure tag column is preserved for tag model training
    if "tag" not in cleaned_df.columns:
    ...
    print("\n[6/6] Saving results...")
    os.makedirs(os.path.dirname(args.cleaned_output), exist_ok=True)
    os.makedirs(os.path.dirname(args.removed_output), exist_ok=True)

    if args.cleaned_output.lower().endswith(".csv"):
        cleaned_df.to_csv(args.cleaned_output, index=False)
    else:
        cleaned_df.to_excel(args.cleaned_output, index=False)

    all_flagged.to_csv(args.removed_output, index=False)
    ...
    print(f"\nCleaned data saved to: {args.cleaned_output}")
    print(f"Removed rows saved to: {args.removed_output}")
    print("\nNext step: Train on the cleaned dataset with:")
    print(f"  python train.py --train_xlsx {args.cleaned_output} --test_size 0.10")
